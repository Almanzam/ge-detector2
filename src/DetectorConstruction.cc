...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
    ...
    cad_logical =
	new G4LogicalVolume(cad_solid, Ge, "cad_logical", 0, 0, 0);
    cad_physical =
	...
	CADMesh *tet_mesh =
	    new CADMesh((char *) filename.c_str(),
			(char *) filetype.c_str());
	tet_mesh->SetScale(1.5);
	tet_mesh->SetMaterial(Ge);	// We have to do this before making the G4AssemblyVolume.
